...
describe('schema artifact generator', () => {
  it('generates bootstrap sql for d1, mysql, and postgres from the contract', () => {
    const artifacts = generateDialectArtifacts(readSchemaContract());

    expect(artifacts.d1Bootstrap).toContain('CREATE TABLE IF NOT EXISTS "sites"');
    expect(artifacts.d1Bootstrap).toContain('CREATE TABLE IF NOT EXISTS "site_disabled_models"');
    expect(artifacts.d1Bootstrap).toContain('"snapshot_key" TEXT NOT NULL');
