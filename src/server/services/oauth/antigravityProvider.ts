...
export const ANTIGRAVITY_CLIENT_ID = config.antigravityClientId;
export const ANTIGRAVITY_CLIENT_SECRET = config.antigravityClientSecret;
...
function requireAntigravityOAuthCredentials(): {
  clientId: string;
  clientSecret: string;
} {
  if (!ANTIGRAVITY_CLIENT_ID || !ANTIGRAVITY_CLIENT_SECRET) {
    throw new Error('ANTIGRAVITY_CLIENT_ID and ANTIGRAVITY_CLIENT_SECRET must be configured');
  }
  return {
...
export const antigravityOauthProvider: OAuthProviderDefinition = {
  ...
  buildAuthorizationUrl: async ({ state, redirectUri }) => {
    const credentials = requireAntigravityOAuthCredentials();
