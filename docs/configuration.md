...
OAuth client secret 必须通过环境变量或部署平台 Secret 配置，不要提交到仓库。

| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| `CODEX_CLIENT_ID` | 覆盖内置 Codex OAuth Client ID | 内置默认值 |
| `CLAUDE_CLIENT_ID` | Claude OAuth Client ID | 空 |
| `CLAUDE_CLIENT_SECRET` | Claude OAuth Client Secret | 空 |
| `GEMINI_CLI_CLIENT_ID` | Gemini CLI OAuth Client ID | 空 |
| `GEMINI_CLI_CLIENT_SECRET` | Gemini CLI OAuth Client Secret | 空 |
| `ANTIGRAVITY_CLIENT_ID` | Antigravity OAuth Client ID | 空 |
| `ANTIGRAVITY_CLIENT_SECRET` | Antigravity OAuth Client Secret | 空 |

说明：

- `Gemini CLI` 和 `Antigravity` 需要同时配置对应的 Client ID 与 Client Secret。
